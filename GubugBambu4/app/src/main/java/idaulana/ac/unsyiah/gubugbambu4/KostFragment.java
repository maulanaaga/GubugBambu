...
public class KostFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View v = inflater.inflate(R.layout.fragment_kost,container,false);

        lv = (GridView) v.findViewById(R.id.grid_view3);
        session = new SharedPreference(getContext());
        Retrofit retrofit = RetrofitHelper.retrofitBuilder();
        WebService webService = retrofit.create(WebService.class);
        Call<ListPilihTokoPOJO> cal = webService.listKost(
                session.getID()
        );
