...
public class TokoFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View v = inflater.inflate(R.layout.fragment_toko,container,false);

        lv = (GridView) v.findViewById(R.id.grid_view4);
        session = new SharedPreference(getContext());
        Retrofit retrofit = RetrofitHelper.retrofitBuilder();
        WebService webService = retrofit.create(WebService.class);
        Call<ListPilihTokoPOJO> cal = webService.listToko(
                session.getID()
        );
